...
def calculate_molecule_3D_structure():
    def get_smiles_list_():
        data_df = pd.read_csv("/data/cjm/human_BoM/data/raw/BoM_singatom_reactl.csv")
    ...
    def calculate_molecule_3D_structure_(smiles_list):
        ...
        while True:
            ...
            if result != 0:
                ...
                mutex.acquire()
                with open('../SRS_bond_data/result/invalid_smiles.txt', 'a') as f:
            ...
            try:
                ...
                mutex.acquire()
                with open('/data/cjm/human_BoM/data/result/invalid_smiles.txt', 'a') as f:
    ...
    mutex = Lock()
    os.system('rm /data/cjm/human_BoM/data/result/invalid_smiles.txt')
    ...
    for t in threads:
        t.join()
    pkl.dump(smiles_to_conformation_dict, open('/data/cjm/human_BoM/data/intermediate/smiles_to_conformation_dict.pkl', 'wb'))
    print('Valid smiles count:', len(smiles_to_conformation_dict))


def construct_data_list():
    data_df = pd.read_csv("/data/cjm/human_BoM/data/raw/BoM_singatom_reactl.csv")
    smiles_to_conformation_dict = pkl.load(open('/data/cjm/human_BoM/data/intermediate/smiles_to_conformation_dict.pkl', 'rb'))
    data_list = []
    ...
    pkl.dump(data_list, open('/data/cjm/human_BoM/data/intermediate/data_list.pkl', 'wb'))

def convert_data_list_to_data_loader():
    def convert_data_list_to_dataset_(data_list):
        dictionary = Dictionary.load('/data/cjm/human_BoM/data/raw/token_list.txt')
    ...
    batch_size = 8  # 原来为8
    data_list = pkl.load(open('/data/cjm/human_BoM/data/intermediate/data_list.pkl', 'rb'))
...
class EsmUnimolClassifier(nn.Module):
    ...
    def forward(self, data_batch):
        ...
        for i in range(len(atom1_embedding)):
            ...
            bond_molecule_embedding.append(th.cat((cls, bond), dim=0))
...
def evaluate(model, data_loader, csv_save):
    ...
    if csv_save == True:
        ...
        df.to_csv('bond_label_without_finetune_mcc.csv', index=False)
...
if __name__ == "__main__":
    ...
    print("train start!")
    train(trial_version='bond_bs16_without_finetune_mcc')  # 表示训练集为全部训练集时的结果

    print("test start!")
    test(trial_version='bond_bs16_without_finetune_mcc')
